      ...
      <Card hasRing={false} className="rounded-2xl border-0 bg-white p-4 shadow-[0_4px_16px_rgba(15,23,42,0.04)] transition active:scale-[0.99]">
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
          <div className="flex flex-col">
            ...
            <span className="mt-1 text-sm font-bold text-emerald-600 leading-none">{rupiah.format(student.saldoUangSaku)}</span>
          </div>
          <div className="flex flex-col text-right">
            <span className="text-xs font-medium text-slate-400">Saldo Dompet:</span>
            <span className="mt-1 text-sm font-bold text-blue-600 leading-none">{rupiah.format(student.saldoDompet)}</span>
          </div>
        </div>
      </Card>
    </Link>
