...
import { fallbackMutabaah } from "../../application/musyrif-fallback-data";
import { useMutabaahList } from "../../application/musyrif-queries";
import { MusyrifHeader } from "../components/musyrif-header";
import { MutabaahCard } from "../components/mutabaah-card";

export function MutabaahListPage() {
  const navigate = useNavigate();
  const [search, setSearch] = useState("");
  const query = useMutabaahList();
  const isFallbackMode = query.isError && isDemoFallbackEnabled();
  const source = useMemo(
    () => query.data ?? (isFallbackMode ? fallbackMutabaah : []),
  ...
  const items = useMemo(() => {
    ...
    return source.filter((item) => {
      return (
        item.santriName?.toLowerCase().includes(keyword) ||
  ...
  return (
    <main className="mx-auto max-w-[430px]">
      <MusyrifHeader
        ...
        onBack={() => navigate("/musyrif/aktivitas")}
      />
          ...
          <div className="flex flex-col gap-4">
            {query.isError ? (
              <Card className="rounded-[22px] border-0 bg-white p-4 text-sm text-amber-700 shadow-sm">
                {isFallbackMode ? "Data contoh ditampilkan karena API belum tersedia." : "Gagal memuat data mutabaah dari API."}
              </Card>
            ) : null}
            {items.map((item) => <MutabaahCard key={item.id} item={item} />)}
            {!items.length ? (
      ...
      <div className="fixed bottom-24 left-0 right-0 pointer-events-none z-30">
        <div className="mx-auto max-w-[430px] w-full px-4 flex justify-end">
          <Button asChild className="pointer-events-auto size-14 rounded-full bg-[#288DE5] hover:bg-[#1E74C5] text-white shadow-lg border-0 shadow-[#288DE5]/20">
            <Link to="/musyrif/aktivitas/mutabaah/tambah">
